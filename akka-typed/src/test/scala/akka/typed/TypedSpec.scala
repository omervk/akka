...
class TypedSpec(val config: Config) extends TypedSpecSetup {
  ...
  val nativeSystem = ActorSystem(AkkaSpec.getCallerName(classOf[TypedSpec]), guardian(), Some(config withFallback AkkaSpec.testConf))
  val adaptedSystem = ActorSystem.adapter(AkkaSpec.getCallerName(classOf[TypedSpec]), guardian(), Some(config withFallback AkkaSpec.testConf))
