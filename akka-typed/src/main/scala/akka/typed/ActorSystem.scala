...
object ActorSystem {
  ...
  def apply[T](name: String, guardianBehavior: Behavior[T],
    ...
    new ActorSystemImpl(name, appConfig, cl, executionContext, guardianBehavior)
  }
  ...
  def adapter[T](name: String, guardianBehavior: Behavior[T],
    ...
    val cl = classLoader.getOrElse(akka.actor.ActorSystem.findClassLoader())
    val appConfig = config.getOrElse(ConfigFactory.load(cl))
    val untyped = new a.ActorSystemImpl(name, appConfig, cl, executionContext, Some(PropsAdapter(guardianBehavior)))
    untyped.start()
