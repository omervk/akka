...
private[typed] class ActorContextAdapter[T](ctx: a.ActorContext) extends ActorContext[T] {
  ...
  override def spawnAnonymous[U](behavior: Behavior[U], dispatcher: DispatcherSelector, mailboxCapacity: Int) = ctx.spawnAnonymous(behavior, dispatcher, mailboxCapacity)
  override def spawn[U](behavior: Behavior[U], name: String, dispatcher: DispatcherSelector, mailboxCapacity: Int) = ctx.spawn(behavior, name, dispatcher, mailboxCapacity)
