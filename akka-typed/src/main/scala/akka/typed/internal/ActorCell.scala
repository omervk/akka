...
private[typed] class ActorCell[T](
  ...
  override def spawn[U](behavior: Behavior[U], name: String, dispatcher: DispatcherSelector, mailboxCapacity: Int): ActorRef[U] = {
    ...
    val cell = new ActorCell[U](system, Behavior.validateAsInitial(behavior), system.dispatchers.lookup(dispatcher), mailboxCapacity, self)
  ...
  override def spawnAnonymous[U](behavior: Behavior[U], dispatcher: DispatcherSelector, mailboxCapacity: Int): ActorRef[U] = {
    val name = Helpers.base64(nextName)
    nextName += 1
    spawn(behavior, name, dispatcher, mailboxCapacity)
  }
