...
private[typed] class EventStreamImpl(private val debug: Boolean)(implicit private val timeout: Timeout) extends EventStream {
  ...
  def startUnsubscriber(sys: ActorSystem[Nothing]): Unit =
    unsubscriberPromise.completeWith(sys.systemActorOf(unsubscriberBehavior, "eventStreamUnsubscriber", DispatcherDefault, 1000))
  ...
  private def setUpStdoutLogger(config: a.ActorSystem.Settings) {
   ...
   */
  class StandardOutLogger extends ActorRef[LogEvent](StandardOutLoggerPath) with ActorRefImpl[LogEvent] with StdOutLogger {
    override def tell(message: LogEvent): Unit =
      if (message == null) throw new a.InvalidMessageException("Message is null")
      else print(message)
   ...
   * INTERNAL API
   */
  @SerialVersionUID(1L) class SerializedStandardOutLogger extends Serializable {
    @throws(classOf[java.io.ObjectStreamException])
  ...
  val UnhandledMessageForwarder = Static[a.UnhandledMessage] {
   ...
   */
  private[akka] def startStdoutLogger(config: a.ActorSystem.Settings) {
  ...
  private[akka] def startDefaultLoggers(system: ActorSystemImpl[Nothing]) {
    ...
    try {
      ...
      if (system.settings.DebugUnhandledMessage)
        subscribe(ActorRef(
          system.systemActorOf(UnhandledMessageForwarder, "UnhandledMessageForwarder", DispatcherDefault, 1000)),
   ...
   * Internal Akka use only
   */
  private[akka] def stopDefaultLoggers(system: ActorSystem[Nothing]) {
   ...
   * INTERNAL API
   */
  private def addLogger(system: ActorSystemImpl[Nothing],
    ...
    val actor = ActorRef(system.systemActorOf(logger.initialBehavior, name, DispatcherFromConfig(system.settings.LoggersDispatcher), 1000))
    import AskPattern._
